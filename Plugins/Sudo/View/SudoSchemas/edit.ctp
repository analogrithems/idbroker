...
<?php
		...
		echo $this->Form->input($model.'.non'.$name, array( 'type' => 'select', 'multiple' => 'true', 'label' => 'Not Allowed', 'options'=>$list['notin']));
		echo "<a href='#' id='".$hrefadd."'> add &gt;&gt;</a>\n";
		echo "</div>\n\n";

		$ucname = ucfirst($name);
		echo "<div id='".$name."Div' class='memberSelect'>\n";
		echo $this->Form->input($model.'.'.$name, array( 'type' => 'select', 'multiple' => 'true', 'label' => 'Allowed', 'options'=>$list['allowed']));
...
<?php
	$user = listPrep( array('notin'=>$sudousers, 'allowed'=>$this->request->data['SudoSchema']['sudouser']));
	selectColumns('sudouser', $form, 'Users & Groups in this Role', $model, $user);

	$group = listPrep(array('notin'=>$sudogroups, 'allowed'=>$this->request->data['SudoSchema']['sudorunasgroup']));
	selectColumns('sudogrunasgroup', $form, 'A Group This Role Can Be Run As', $model, $group);

	$computer = listPrep( array('notin'=>$sudohosts, 'allowed'=>$this->request->data['SudoSchema']['sudohost']));
