...
class LdapSource extends DataSource {
    ...
    function connect() {
		...
		$hasFailOver = false;
		//If The configured more than one ldap server
		if(isset($config['host']) && is_array($config['host']) && (count($config['host']) > 1) ){
		...
		if(!$this->database){
		    $this->log("Failed to connect to LDAP Server:".$config['host'],'ldap.error');
		    //Try Next Server Listed
		    if($hasFailOver){
			    $this->_multiMasterUse++;
			    $this->log("Trying Next Ldap Server:".$this->config['host'][$this->_multiMasterUse],'ldap.error');
			    $this->connect();
			    if($this->connected){
		...
		if (ldap_bind($this->database, $bindDN, $bindPasswd)) $this->connected = true;
		return $this->connected;
    }
            ...
            foreach ($model->__associations as $type) {
                ...
                foreach ($model->{$type} as $assoc => $assocData) {
                    ...
                    if (isset ($db) && $db != null) {
                        $stack = array ($assoc);
                        $array = array ();
                        $db->queryAssociation($model, $linkModel, $type, $assoc, $assocData, $array, true, $resultSet, $model-%