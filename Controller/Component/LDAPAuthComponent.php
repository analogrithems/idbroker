		...
		switch ($type) {
			...
			case 'actions':
				if (isset($controller->LDAPAcl)) {
					...
					trigger_error(__('Could not find LDAPAclComponent. Please include LDAPAcl in Controller::$components.'), E_USER_WARNING);
				}
			break;
			case 'model':
				if (!isset($object)) {
 ...
 */
	function isAuthorized($type = null, $object = null, $user = null) {
		...
		switch ($type) {
			...
			break;
			case 'crud':
				if (!isset($this->actionMap[$this->request->params['action']])) {
					trigger_error(
						sprintf(__('Auth::startup() - Attempted access of un-mapped action "%1$s" in controller "%2$s"'), $this->request->params['action'], $this->request->params['controller']),
						E_USER_WARNING
					);
				} else {
					$valid = $this->LDAPAcl->check(
						...
						$this->actionMap[$this->request->params['action']]
					);
				}
			break;
			case 'model':
	...
	function identify($user = null, $conditions = null) {
		...
		if (is_array($user) && (isset($user[$this->fields['username']]) || isset($user[$this->model->alias . '.' . $this->fields['username']]))) {
			...
			if( $loginResult == 1){
				...
				$this->Session->setFlash(__('Invalid Username or Password, Please try again.'), 'default',array('class'=>'error-message'));
